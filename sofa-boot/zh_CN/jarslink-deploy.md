在 Jarslink2.0 中，对合并部署的定义是指在同一个 JVM 中，加载运行多个 Biz 包。在[应用打包](.)这一节中，我们描述了 Spring Boot/SOFABoot 应用和 Biz 包的关系。因此也可以认为，这里的合并部署是指在同一个 JVM 中，加载运行多个 Spring Boot/SOFABoot 应用。
